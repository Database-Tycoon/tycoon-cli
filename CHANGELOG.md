...
- **`tycoon-city` renders its help like every other tycoon command.** The renderer's own console script is now a Typer app (`tycoon_city.cli`) with `serve` and `demo` subcommands — `tycoon-city PATH` stays legal as shorthand for `serve` — so `tycoon-city demo --help` and `tycoon city --help` no longer look like two different tools (one argparse, one Typer). The argparse `python -m tycoon_city.webserve [demo]` path remains for Docker and CI; both surfaces share one implementation (`webserve.run_server`, `demo.cli.run`).
