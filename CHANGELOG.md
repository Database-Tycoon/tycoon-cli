...
- **Tycoon observability: auto-generated dlt + dbt run-history dashboards in Rill** ([#13]). Adds a dedicated metadata DuckDB at `.tycoon/metadata.duckdb` (disposable — safely delete to reset history) with four tables: `dlt_runs`, `dlt_rows_by_table`, `dbt_runs`, `dbt_nodes`. Writes are idempotent (`INSERT … ON CONFLICT DO NOTHING`), so every capture is safe to re-run. Capture hooks: (a) the dlt ingestion runner mirrors each schema's `_dlt_loads` + per-table `_dlt_load_id` counts into the metadata DB after every successful `tycoon data sources run`; (b) `tycoon data transform run/test/build` parses `target/run_results.json` after each dbt invocation and inserts one row into `dbt_runs` plus one row per model/test into `dbt_nodes`. Both captures are best-effort (wrapped in try/except) so observability hiccups never break ingestion or dbt runs. Two Rill dashboards are auto-emitted whenever the corresponding table is non-empty: `_tycoon_dlt_usage.yaml` (measures: total loads, success rate, rows loaded; dimensions: schema, table, status, load_id, schema_version) and `_tycoon_dbt_usage.yaml` (measures: runs, success rate, avg duration, models built, model errors, tests passed/failed; dimensions: command, target, dbt version, invocation_id). Parquet snapshots under `data/parquet/_tycoon/` are re-exported after every capture so Rill stays current without a manual rescaffold. The metadata DB itself can be queried directly: `tycoon data query --db .tycoon/metadata.duckdb "SELECT * FROM dbt_runs ORDER BY started_at DESC"`. Caveat: dlt per-load row counts derive from `count(*) GROUP BY _dlt_load_id` — exact for append-only, best-effort for replace/merge (only the most recent load's counts are accurate). dlt byte sizes + per-job durations (via `trace.json`) and dbt manifest-based schema-diff are deferred to a follow-up.
