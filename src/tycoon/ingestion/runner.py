    ...
    **kwargs: Any,
    ...
    try:
        ...
        if builder is None:
            ...
            if source_type == "filesystem" and not isinstance(dlt_source, list):
                # Multi-resource sources (gh-224) come back as a list of
                # resources already named after their own table_name; only
                # the older single-resource shape still needs renaming after
                # the tycoon source itself here. See _build_filesystem_source
                # and _build_filesystem_resource. Issue #222.
