...
Generated files include a machine-readable sentinel on the first line so
:func:`generate_staging_models` can safely re-run without clobbering files
the user has hand-edited. Removing the sentinel takes ownership of the file.
"""
...
class GenerateResult(NamedTuple):
    """Outcome of :func:`generate_staging_models`.

    ``generated`` lists paths that were (re)written; ``skipped`` lists
    paths that were left alone because the user removed the sentinel
    (i.e. took ownership). Use ``--force`` to regenerate skipped files.
    """
...
# Sentinel embedded in every generated file's first ~200 chars. Hand-edited
# files have this removed; safe-write checks for it before overwriting.
SQL_SENTINEL = "-- @generated by tycoon analyze"
YAML_SENTINEL = "# @generated by tycoon analyze"

# How many bytes from the top of a file we'll scan for the sentinel.
_SENTINEL_SCAN_BYTES = 500
...
def _is_safe_to_overwrite(path: Path, sentinel: str) -> bool:
    """Return True if ``path`` doesn't exist or still carries the sentinel.

    A missing sentinel means the user has either hand-authored the file or
    deliberately removed the marker to take ownership; in either case we
    must not silently clobber it.
    """
    if not path.exists():
        return True
    head = path.read_text(errors="replace")[:_SENTINEL_SCAN_BYTES]
...
-- Delete the sentinel above to take ownership; tycoon will then leave
-- this file alone on subsequent `tycoon data analyze` runs (use --force
-- to override and regenerate).
...
def generate_staging_models(
    ...
    *,
    ...
    Files that already exist without the tycoon sentinel are treated as
    user-owned and skipped (logged via the returned ``skipped`` list).
    ...
    if not raw_table_names:
        return GenerateResult(generated=[], skipped=[])
    ...
    # Write one .sql file per table — skip if user removed the sentinel.
    for table_name, columns in tables.items():
        model_name = f"stg_{source_name}__{table_name}"
        sql_path = output_dir / f"{model_name}.sql"
        if not force and not _is_safe_to_overwrite(sql_path, SQL_SENTINEL):
            skipped.append(str(sql_path))
            continue
    ...
    if tables:
        ...
        if not force and not _is_safe_to_overwrite(yaml_path, YAML_SENTINEL):
