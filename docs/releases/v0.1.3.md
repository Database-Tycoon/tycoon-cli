...
- No user-facing bug fixes this cycle — v0.1.2's fix-sweep covered
  the pending bugs; v0.1.3 is a feature release.
...
- Pre-existing `ty` type diagnostics in
  `src/tycoon/ingestion/runner.py` at lines 84 (dlt
  `rest_api_source` expecting `RESTAPIConfig` not `dict`) and 273
  (regex `match.group` on `None`). Line numbers shifted slightly
  from v0.1.2 as the runner picked up the trace-capture plumbing.
- `src/tycoon/scaffolding/rill_generator.py` line 210:
  `warehouse_db_path` param is accepted but unused inside
  `generate_rill_config`. Either wire it in or deprecate.
