...
- **Coverage floor**: CI now fails if overall coverage drops below 55%.
  Baseline is ~61%, so there's ~6% drift headroom. The floor lives in
...
- **Coverage gate** via `pytest-cov`: floor at 55% in
  `[tool.coverage.report].fail_under`; baseline ~61%. CI uploads
