...
Five themes in this release: **MotherDuck alignment**,
**`tycoon register warehouse`**, a **network-gated e2e test suite**, a
**Nao/ask cleanup pass**, and a new **Tycoon observability layer** that
captures dlt + dbt run history into a dedicated metadata DuckDB and
surfaces it as two auto-generated Rill dashboards — finishing out the
register family, the warehouse-alignment story from v0.1.1, fixing a
cluster of papercuts around `tycoon ask` that surfaced during the
MotherDuck + Nao + LM Studio dogfood walkthrough, and starting to close
the observability gap between "I ran a pipeline" and "I can see exactly
what it did, now and historically".
