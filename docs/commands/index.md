# Commands

Every command tycoon ships, organized by section. Click into any
command for full reference. For a flat command-by-command index with
every flag listed, see [Reference: All commands](../reference/all-commands.md).

## Project

| Command | What it does |
|---|---|
| [`tycoon init`](init.md) | Scaffold a new project (wizard or `--template <name>`). |
| [`tycoon register dbt`](register.md#tycoon-register-dbt) | Attach an existing dbt project; `--profiles-dir`, `--profile`, `--target` persist into `tycoon.yml`. |
| [`tycoon register rill`](register.md#tycoon-register-rill) | Attach an existing Rill project. |
| [`tycoon register warehouse`](register.md#tycoon-register-warehouse) | Switch warehouse (DuckDB / MotherDuck); `--type`, `--path`, `--catalog`, `--no-prompt`, `--force` for scripted use. |
| [`tycoon doctor`](doctor.md) | Cross-component health check (dbt, warehouse, observability). |
| [`tycoon docs serve`](docs.md) | Build + serve this docs site locally with hot reload. |
| [`tycoon docs build`](docs.md) | One-shot build into `site/`. |

## Data pipeline

| Command | What it does |
|---|---|
| [`tycoon data sources catalog`](data/sources.md#catalog-browse-available-source-types) | Browse available source types. |
| [`tycoon data sources add`](data/sources.md#add-register-a-new-source) | Register a new source interactively. |
| [`tycoon data sources list / show`](data/sources.md#list-list-registered-sources) | List registered sources / inspect one. |
| [`tycoon data sources run [name]`](data/sources.md#run-ingest) | Ingest one source. Auto-scaffolds dbt staging models when no models reference the source yet (opt out with `--no-scaffold` or project-wide `transform.auto_scaffold: false`). |
| [`tycoon data sources run-all`](data/sources.md#run-ingest) | Ingest every registered source sequentially. |
| [`tycoon data sources remove <name>`](data/sources.md#remove-unregister-a-source) | Remove a registered source. |
| [`tycoon data transform run / test / build`](data/transform.md) | `dbt run` / `test` / `build` against the project's persisted profile + target. |
| `tycoon data transform docs` | `dbt docs generate` then serve. |
| [`tycoon data sync`](data/sync.md) | Cloud → local DuckDB snapshots; `replace` / `append` / `skip-existing` modes. |
| [`tycoon data query`](data/query.md) | Read-only SQL against the warehouse. |
| [`tycoon data schema`](data/query.md#schema-dump-tables-row-counts-sizes) | Dump tables, row counts, file sizes. |
| [`tycoon data clean`](data/query.md#clean-remove-warehouse-files) | Remove warehouse files (preserves observability metadata by default). |
| [`tycoon data history`](data/history.md) | Recent dlt + dbt run history captured in `.tycoon/metadata.duckdb`. |
| [`tycoon data history show <id>`](data/history.md#show-id-drill-into-one-run) | Drill into one run. |
| [`tycoon data status`](data/status.md) | Source freshness, last sync time, row counts. |
| [`tycoon data analyze [source]`](data/analyze.md) | Auto-scaffold dbt staging models from a source's dlt schema. Sentinel-protected (`@generated by tycoon analyze`); `--force` to clobber hand-edits, `--all` to iterate every registered source. Auto-triggers after `data sources run` by default. |
| `tycoon data observability scaffold` | Retrofit the `_tycoon` staging views + metadata-DB ATTACH onto an existing dbt project. Idempotent. |
| [`tycoon data run-all`](data/run-all.md) | Ingest all sources, then `dbt build`. The full-pipeline shortcut. |
| [`tycoon data layers`](data/layers.md) | The layered architecture ring by ring, with vendor per object (dlt / Fivetran / dbt). |
| [`tycoon data health`](data/layers.md#tycoon-data-health) | The city's health strip as text: one count per problem class. |
| [`tycoon data sources explore`](data/sources.md) | Inspect a source's schema, sample rows, and health. |

## Fire & Response

| Command | What it does |
|---|---|
| [`tycoon fire`](fire.md) | What's burning: failing tests, one per building on fire in the city. |
| [`tycoon repair`](fire.md#tycoon-repair) | The contractor call sheet: sources past their freshness SLA. |
| [`tycoon firehouse`](fire.md#tycoon-firehouse) | Dispatch stats: trucks and vans on duty. |

## Services

| Command | What it does |
|---|---|
| [`tycoon start`](start.md) | Start the Rill dashboards and the Quack warehouse server. |
| [`tycoon start --only <svc>`](start.md) | Start one specific service. |
| `tycoon stop` | Stop all services started by `tycoon start`. |
| [`tycoon city`](city.md) | Serve the catalog as an interactive 3D city in the browser. |

## Tool passthrough

| Command | What it does |
|---|---|
| `tycoon run dlt ...` | Pass through to dlt CLI. |
| `tycoon run dbt ...` | Pass through to dbt CLI. |
| `tycoon run rill ...` | Pass through to Rill CLI. |

## Help anywhere

Every command supports `--help` (or the `-h` short alias):

```bash
tycoon data sync --help
tycoon register dbt -h
tycoon doctor --help
```
